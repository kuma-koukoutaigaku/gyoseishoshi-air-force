...
class CustomQuestionLoader {
    ...
    parseAndRegister(csv, tabDifficulty) {
        ...
        for (const row of dataRows) {
            let category = row[0] || '';
            let section = (row[1] || '').trim();
            // 分野「行政法」でセクションが個別法名の場合、セクションをカテゴリに昇格
            if (category === '行政法' && section && this.isAdminSubCategory(section)) {
                category = section;
                section = '';
            }
        ...
        for (const catName of categoryOrder) {
            ...
            CATEGORIES[key] = {
                label: catName,
                group: group || (CATEGORIES[key] && CATEGORIES[key].group) || null
            };
        }
    ...
    isAdminSubCategory(name) {
        const subs = ['行政代執行法','行政手続法','行政不服審査法','行政事件訴訟法','国家賠償・損失補償','地方自治法'];
